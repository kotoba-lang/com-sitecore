(ns sitecore.main
  "Kotodama WASM entrypoint for the Sitecore clean-room actor (L4) — Clojure port.
  ...
  facts (`sitecore.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sitecore")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sitecore.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sitecore-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sitecore-compat/ready)
